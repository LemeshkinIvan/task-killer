...
func (w *Win32Watcher) StartWatcher(blacklist []string) (metrics.WatcherMetrics, error) {
	var res metrics.WatcherMetrics = metrics.WatcherMetrics{}

	if len(blacklist) == 0 {
		return res, ErrBlacklistLen
	}
	...
	if pr == nil {
		return res, fmt.Errorf("procces is nil")
	}
